...
A key press on the keyboard is received one bit at a time over the serial clock/data lines, reassembled into a scancode byte, and displayed as two hexadecimal digits on the board's 7-segment display. The display updates on every received scancode byte.
